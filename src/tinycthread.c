...
#if defined(_TTHREAD_WIN32_)
int _rdk_thread_cond_timedwait_win32(cnd_t *cond, mtx_t *mtx, DWORD timeout)
{
...
{
#if defined(_TTHREAD_WIN32_)
  return _rdk_thread_cond_timedwait_win32(cond, mtx, INFINITE);
#else
...
int rdk_thread_cond_timedwait(cnd_t *cond, mtx_t *mtx, const struct timespec *ts)
{
  ...
  if (timespec_get(&now, TIME_UTC) == TIME_UTC)
  {
    ...
    return _rdk_thread_cond_timedwait_win32(cond, mtx, delta);
  }
  else
    return thrd_error;
#else
...
void *rdk_thread_key_get(tss_t key)
{
#if defined(_TTHREAD_WIN32_)
  struct TinyCThreadTSSData* data = (struct TinyCThreadTSSData*)TlsGetValue(key);
  if (data == NULL)
...
int rdk_thread_key_set(tss_t key, void *val)
{
#if defined(_TTHREAD_WIN32_)
  struct TinyCThreadTSSData* data = (struct TinyCThreadTSSData*)TlsGetValue(key);
  if (data == NULL)
